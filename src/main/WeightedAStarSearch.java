...
public class WeightedAStarSearch extends InformedSearch {

	WeightedAStarSearch(Grid grid, Panel panel) {
		super(grid, panel);
	...
	int evaluate(Node n) {
		...
		int w = 2; //weight
		int i=heuristic(n.state, grid.goalState)*w + n.state.pCost;
		return i;
	}
	...
	int heuristic(State a, State b) {
		...
		return (int) Math.abs(Math.sqrt(Math.pow(aSide, 2) + Math.pow(bSide, 2)));
	}
